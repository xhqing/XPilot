...
## 🛠️ XPilot —— x-ray pilot，Hermes 开发的 Xray 工具

**XPilot** 是一个方便使用 [Xray-core](https://github.com/XTLS/Xray-core) 的 Python CLI 工具，是**网络运维管理员 Agent Hermes（赫尔墨斯）**开发的 CLI 工具之一，作为 [NetOpsAgent](https://github.com/xhqing/NetOpsAgent) 仓库内的独立子项目存在。xpilot 即「x-ray pilot」，是 Xray 的驾驶工具。Hermes 的总体职责是处理**网络连接相关的问题**——代理转发只是众多网络问题中的一部分，而 XPilot 正是 Hermes 为覆盖「代理」这一块而打造的工具。

XPilot 保障你的代理连接稳定、快速：
...
本文档独立介绍 XPilot。你可以自己执行命令，也可以交给 AI 助手——无论哪种方式，干活的都是这个工具。
...
克隆仓库后，进入 `XPilot` 子项目目录从源码安装（开发模式）：
...
从项目的 [GitHub Release](https://github.com/xhqing/NetOpsAgent/releases) 检查最新版本；若比当前版本新，则自动下载并安装（优先用 Release 附带的 wheel，其次 sdist 压缩包，两者都没有时回退到从对应 tag 的源码安装）。
...
安装一个更早的 [GitHub Release](https://github.com/xhqing/NetOpsAgent/releases)。不带 `--version` 时，回滚到严格早于当前版本的最近一个 Release；带 `--version X.Y.Z` 时，安装指定的那个版本。安装复用与 `--update` 相同的资产（优先 wheel，其次 sdist 压缩包，两者都没有时回退到对应 tag 的源码）。
